...
仓库根目录就是 skill 目录,直接克隆到工具读取 skill 的位置即可(文件夹名必须是 `efmi-sheer-skin`):

```bash
git clone https://github.com/hehex4/efmi-sheer-skin.git ~/.claude/skills/efmi-sheer-skin
```

支持 Agent Skills 公共目录的工具改用 `~/.agents/skills/efmi-sheer-skin`。用网页上的 Download ZIP 也可以,解压后把 `efmi-sheer-skin-main` 改名为 `efmi-sheer-skin`。之后的依赖安装(Python + numpy + Pillow)和编码设置见 [INSTALL.md](INSTALL.md)。
