...
得到每个目标状态的完整 draw 入口、资源绑定、材质 PS hash 和 LOD 覆盖范围，并用这些证据预填第二轮三问。

## 进入条件

- [ ] 步 00 已取得 U1、U2；U2 文件名中的 PS hash 已记录。
- [ ] 步 01 已判定 B / C / A 肤色路线，并列出疑似丝袜 draw。
- [ ] 只读定位可以开始；Q1–Q3 未答齐时仍不修改 mod、不生成替换 PS。
...
| `<目标 PS hash>` | U2 文件名中的 16 位 PS hash | 步 00 U2 | `0123456789abcdef` |
...
| `<LOD0 转储>` | 主控帧目录或它的 `log.txt` | 用户可选提供 | `E:\EMFI\FrameAnalysis\... [mod]` |
...
4. 没有帧转储时，保留“材质 pass 只绑目标 hash”为实机待证项，继续完成 ini 侧定位。用户给了帧转储时，先用段名核对 draw、命令与槽位：
...
6. 从同一 draw 的多次绘制中找材质 pass：看 `--cmds` 展开的条件，材质 VS 门控为 true 的那次，记录它的 VS / PS。阴影、prepass、描边会有相同 draw 参数，但 VS / PS 不同。
...
8. 把读到的状态变量、`key =`、默认值、每个值对应的 draw / diffuse、丝袜片清单写进 Q2 / Q3 的预填；Q1 保留为用户的 hunting 观察。
9. ⏸ 在这里发送步 00 的 Q1–Q3，一条回复问完。Q1–Q3 全答清才进步 04；等待期间可以并行完成步 03 的两条只读门控审计。
...
| 看到什么 | 结论 | 下一步 |
|---|---|---|
| 输出明确列出状态条件、默认状态、draw 与漫反射 | ini 侧定位完整 | 继续逐片预览并预填 Q2 / Q3 |
...
| Q1–Q3 全部明确 | 范围与状态门通过 | 步 03 审计已完成时进步 04；否则先完成步 03 |
...
| Q1–Q3 用户答复 | 当前会话中的已确认输入 | 步 04 起的正式施工 |
